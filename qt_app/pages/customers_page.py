"""Customers page — μητρώο πελατών (read-only Phase A)."""
...
class _CustomerWorker(QObject):
    finished = Signal(CustomerPageResult)
    ...
    def run(self):
        self.finished.emit(load_customers_page(*self._args))


class CustomersPage(BasePage):
    shutdown_ready = Signal()

    COLS = ["Όνομα", "ΑΜΚΑ", "Τηλέφωνο", "Παραστατικά", "Σύνολο Αγορών"]
    ...
    def build_ui(self):
        ...
        self._search.setPlaceholderText(
            "Αναζήτηση ονόματος, ΑΜΚΑ ή τηλεφώνου…")
    ...
    def _do_refresh(self):
        ...
        self._set_state("🔄 Φόρτωση πελατών...", styles.TEXT_MUTED)
        self._cleanup_worker()
        self._thread = QThread(self)
        self._worker = _CustomerWorker(
    ...
    def _on_ready(self, result):
        ...
        self._summary.setText(f"Εμφάνιση {first}–{last} από {total} πελάτες")
        items = result.items
        if not items:
            self._set_state("Δεν βρέθηκαν πελάτες.", styles.TEXT_MUTED)
            self._table.setRowCount(0)
        else:
            ...
            for r, c in enumerate(items):
                ...
                self._table.setItem(r, 1, QTableWidgetItem(c.amka))
                self._table.setItem(r, 2, QTableWidgetItem(c.phone))
                self._table.setItem(r, 3, QTableWidgetItem(str(c.invoice_count)))
                self._table.setItem(r, 4, QTableWidgetItem(f"€{c.total_purchases:.2f}"))
    ...
    def _show_detail(self):
        ...
        if len(rows) != 1:
            return
        r = list(rows)[0]
        cid = self._table.item(r, 0).data(Qt.UserRole)
        if cid is None:
            QMessageBox.warning(self, "Σφάλμα", "Αδυναμία εύρεσης πελάτη.")
            return
        detail = load_customer_detail(self._db_path, cid)
        if not detail.ok:
            QMessageBox.warning(self, "Σφάλμα", detail.error_message)
            return
        self._show_detail_dialog(detail.customer)

    def _show_detail_dialog(self, c):
        dlg = QDialog(self)
        dlg.setWindowTitle(f"Πελάτης: {c.name}")
        dlg.setMinimumWidth(400)
        lay = QFormLayout(dlg)
        fields = [
            ("Όνομα", c.name), ("ΑΜΚΑ", c.amka), ("Τηλέφωνο", c.phone),
            ("Παραστατικά", str(c.invoice_count)),
            ("Σύνολο Αγορών", f"€{c.total_purchases:.2f}"),
            ("Τελευταίο Παραστατικό", c.latest_invoice_date),
        ]
